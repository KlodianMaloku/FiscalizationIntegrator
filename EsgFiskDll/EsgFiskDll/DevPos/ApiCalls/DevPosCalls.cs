...
namespace EsgFiskDll.DevPos.ApiCalls
{
    ...
    internal class DevPosApiCalls
    {
        ...
        public string doRegisterCashDesk(SqlConnection Conn, FiskConfigs Configs, Int32 CashDeskId, Int32 UserId)
        {
            ...
            try
            {
                //DateTime validToDate = DateTime.ParseExact(Configs.ServerDatetime, "yyy-MM-ddThh:mm:sszz:zz", CultureInfo.InvariantCulture);
                RegisterCashDeskReq body = new RegisterCashDeskReq()
                {
                    businesUnitcode = Configs.BusinUnitCode, 
                    validFromDate = "2023-04-08T18:55:59+02", //Configs.ServerDatetime,
                    validToDate = "2024-04-08T18:55:59+02", // validToDate.AddYears(1).ToString(),
                };
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            body.tcrIntID = 4; // (string)reader["KODI"];
                            body.type = 0;
                            body.name = (string)reader["KODI"];
                        }
                    }
                    else
                    {
                ...
                string url = Configs.url + "/TCR/int/create";
        ...
        public string doRegisterCashDeposit(SqlConnection Conn, FiskConfigs Configs, Int32 CashDepositId, Int32 UserId)
        {
            try
            {
                RegisterCashDepositReq body = new RegisterCashDepositReq();
                    ...
                    using (SqlDataAdapter sda = new SqlDataAdapter(command))
                    {
                        ...
                        if (ds.Tables.Count != 0)
                        {
                            if (ds.Tables[0].Rows.Count != 0)
                            {
                                ...
                                body.operatorCode = ds.Tables[0].Rows[0]["operatorCode"].ToString();
                                body.Value = (double)ds.Tables[0].Rows[0]["Amt"];
                                body.changeDateTime = Configs.ServerDatetime;
                                body.Operation = constants.cashDepositTypes[ds.Tables[0].Rows[0]["Operation"].ToString()];
                                //body.subsequentDeliveryType = ds.Tables[0].Rows[0]["subsequentDeliveryType"].ToString();

                            }
                            else
                            {
                                throw new Exception("Esg Error :" + "Veprimi i arkes nuk u gjet..");
                            }
                        }
                    }
                }

                string url = Configs.url + "/TcrBalance";
